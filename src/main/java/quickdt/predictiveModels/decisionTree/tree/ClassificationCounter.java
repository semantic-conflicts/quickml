...
public class ClassificationCounter implements Serializable {
  public static long serialVersionUID=-6821237234748044623L;
  public ValueSummingMap<Serializable> counts=new ValueSummingMap<Serializable>();
  ...
  public static Pair<ClassificationCounter,Map<Serializable,ClassificationCounter>> countAllByAttributeValues(  final Iterable<? extends AbstractInstance> instances,  final String attribute,  String splitAttribute,  Serializable splitAttributeValue){
    ...
    for (    final AbstractInstance instance : instances) {
      ...
      if (attrVal != null)       cc=result.get(attrVal);
 else       if (acceptableMissingValue)       cc=result.get(MISSING_VALUE);
 else       continue;
      if (cc == null) {
    ...
    double total=0;
    for (    ClassificationCounter cc : result.values())     total+=cc.getTotal();
  ...
  public static Pair<ClassificationCounter,List<AttributeValueWithClassificationCounter>> getSortedListOfAttributeValuesWithClassificationCounters(  final Iterable<? extends AbstractInstance> instances,  final String attribute,  String splitAttribute,  Serializable splitAttributeValue,  final Serializable minorityClassification){
    ...
    Collections.sort(attributesWithClassificationCounters,new Comparator<AttributeValueWithClassificationCounter>(){
      @Override public int compare(      AttributeValueWithClassificationCounter cc1,      AttributeValueWithClassificationCounter cc2){
  ...
  public static boolean isAnAcceptableMissingValue(  AbstractInstance instance,  String splitAttribute,  Serializable splitAttributeValue){
